...
This plan is written so that a TA, professor, or engineer modifying the code can run and evaluate
...
● Development team members: execute unit tests, integration checks, and manual
regression throughout development.
● TA/Professor: execute acceptance and grading checks using the concrete manual test
scripts and expected outcomes defined in this document.
...
● teamService, questionService, sponsorService: create/subscription/update
behavior
...
2.2 Integration/System Testing (Manual UI + Data
Validation)
Fan flow tests
...
● TA/Professor (grader): verifies acceptance outcomes against documented expected
results
...
● Backend test setup: Firebase SDK calls mocked in Jest unit tests (no emulator required
for these service-level tests)
...
● Acceptance: each in-scope user story has documented pass/fail result and expected
outputs verified by TA/Professor.
● Release readiness: no blocking defects in auth, question submission, session control,
or reward claim flows.
...
● Exact software versions must be filled in before final submission to keep grading
reproducible.
11) TA/Professor Execution Script (Concrete Manual
Tests)
...
● Preconditions: Admin credentials available; at least 2 teams, 1+ questions, 1 sponsor
exist.
● Input/Steps:
○ Log in as admin.
